...
func makeJSLayer() {
	...
	println(context.evaluateScript("var layer = new Layer(Layer.root, 'foo'); layer.image = new Image('paint'); layer.frame = {x: 75, y: 80, width: 40, height: 40}; layer.border = new Border({color: Color.black, width: 2})"))
}